...
ipcMain.handle('ipc-cloud-download', async (_, confidentialId: number) => {
  const db = await openDb();
  const account = await db.get('SELECT * FROM accounts WHERE confidential_id = ?', confidentialId);
  const cloud = await db.get('SELECT * FROM clouds WHERE id = ?', account.cloud_id);
  const links = await db.all('SELECT * FROM links WHERE confidential_id = ?', confidentialId);
